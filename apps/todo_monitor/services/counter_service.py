...
class CounterCompletedTaskHandlerService:
    ...
    async def process(self, data: CompletedTaskDTO) -> None:
        user_info = await self.get_or_create(data.user_id)

        if data.duration_in_days > self._max_day_task:
            await self.handle_failed_task(user_info)
        else:
            await self.handle_successful_task(user_info)

    async def get_or_create(self, user_id: int) -> UserTaskOrm:
    ...
    async def handle_failed_task(self, user_info: UserTaskOrm) -> None:
    ...
    async def handle_successful_task(self, user_info: UserTaskOrm) -> None:
